...
const dropDownCloneContainerRef = ref<HTMLElement | null>(null);
...
const setClonePosition = (elemToClone: HTMLElement) => {
  if (!cloneToDrag.element || !dropDownCloneContainerRef.value) {
    return;
  }

  const rectSelectedItem = elemToClone.getBoundingClientRect();
  const rectContainer = dropDownCloneContainerRef.value!.getBoundingClientRect();
  ...
  cloneToDrag.x = rectSelectedItem.x - rectContainer.x;
  cloneToDrag.y = rectSelectedItem.y - rectContainer.y;
};

// mouse down on item to drag
// parameter emited from the list element clone
const onSelectedItemToDrag = (event: MouseEvent, item: ItemType, elemToClone: HTMLElement) => {
  event.preventDefault(); // prevent to select text when mouse move while mouse down

  dropDownCloneContainerRef.value!.addEventListener("mousemove", mouseMove);
  dropDownCloneContainerRef.value!.addEventListener("mouseup", onMouseUp);
...
const onMouseUp = () => {
  ...
  dropDownCloneContainerRef.value!.removeEventListener("mousemove", mouseMove);
  dropDownCloneContainerRef.value!.removeEventListener("mouseup", onMouseUp);
};
...
<template>
  <div
    ...
    <section
      ...
      <list-element-clone
        v-for="item in list1"
        @one-mouse-down-on-item="onSelectedItemToDrag"
        :key="item.id"
        :item="item" />
    </section>
